        ...
        <form method="post" @submit.prevent="submit" class="form-control d-flex flex-column">
            ...
            <select class="form-control" v-model="form.category_id" name="category_id" >
                <option v-for="category in categories"
                        :value="category.id"
                        :selected="category.id == '1'"> {{category.title}} </option>
            </select>
...
export default {
    setup: function () {
        ...
        function submit() {
            this.$inertia.post(route('newsStoreA', {_token: this.$page.props.csrf_token, title: form.title, text: form.text, category_id: form.category_id}))
        }
